...
get_tablesample_method_func(DefElem *defel, int attnum)
{
	...
	switch (attnum)
	{
		...
		case Anum_pg_tablesample_method_tsmgetnext:
			nargs = 1;
			typeId[0] = INTERNALOID;
...
makeTablesampleMethodDeps(HeapTuple tuple)
{
	...
	referenced.objectId = tsm->tsmgetnext;
	recordDependencyOn(&myself, &referenced, DEPENDENCY_NORMAL);
...
DefineTablesampleMethod(List *names, List *parameters)
{
	...
	foreach(pl, parameters)
	{
		...
		else if (pg_strcasecmp(defel->defname, "getnext") == 0)
		{
			values[Anum_pg_tablesample_method_tsmgetnext - 1] =
				get_tablesample_method_func(defel,
					Anum_pg_tablesample_method_tsmgetnext);
		}
	...
	if (!OidIsValid(DatumGetObjectId(values[Anum_pg_tablesample_method_tsmgetnext - 1])))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
				 errmsg("tablesample method getnext function is required")));
